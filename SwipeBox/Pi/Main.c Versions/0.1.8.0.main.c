...
*	Version: 0.1.8.0 (Major.Minor.Revision.Bug)
*	Date: 11/05/2015
**********************************************************/
...
#include <fcntl.h>
#include <string.h>
#include <ph_NxpBuild.h>		// Controls build behaviour of components
...
#define  NDEF_MESSAGE_START  0x9  // ndef message offset from beginning of card memory
...
uint32_t DetectMifare(void *halReader)
{
	...
	while (bMoreCardsAvailable)	{
		...
		if (!status && detected_card == 0xFFFF) {
			...
			// check if it is actually a phone and try and receive the NDEF record
		...
		// NDEF
		uint8_t bNDEFBuffer[48];	// buffer for the NDEF data
		phalMful_Read(&alMful, 4, bNDEFBuffer);	// reading the data from the card into the buffer
		const int indexFinish = 15;	// 
		unsigned char bCusID[5];
		...
		printf("\n");
		// copies the cusID part of the NDEF record
		// memcpy(destination, source, size)
		memcpy(bCusID, &bNDEFBuffer[NDEF_MESSAGE_START], indexFinish - NDEF_MESSAGE_START);
		...
		// Write extracted cusID to txt file
		WriteToFile(bCusID);
		
		// Use the system to run the ServiceClient.py Python program
		system("python ServiceClient.py");
...
// Write the cusID from the NFC card or phone to a txt file
void WriteToFile(unsigned char pID[])
{
	printf("Writing to file. . . . .\n");
	FILE *f = fopen("customerID.txt", "a");
	...
	fprintf(f, "Customer ID: %s \n", pID);

	fclose(f);
	
	printf("The cusID %s has been written to the file\n", pID);
}
