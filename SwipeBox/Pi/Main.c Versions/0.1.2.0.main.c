...
void DoALoop()
{
	...
	state = AuthenticateBlocks(_UID, address, keyAccess);
	fprintf(stderr,"\n");
...
void CheckSum(uint8_t *dataTX)
{
	...
	unsigned char byte = dataTX[dataTX[0] + 2]= - dataTX[dataTX[0] + 2];
}

// Calculates the length checksum and stores it in the buffer.
uint8_t LengthCheckSum(uint8_t *dataTX)
{
	printf("LengthCheckSum\n");
	unsigned char byte = 0x100 - dataTX[0];
	dataTX[1] = byte;
}
